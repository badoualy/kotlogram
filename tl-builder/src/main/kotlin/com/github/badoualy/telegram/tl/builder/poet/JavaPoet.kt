...
import com.github.badoualy.telegram.tl.builder.utils.*
...
//            //val nonEmptyConstructor = if (!forEmptyConstructor) null else typeConstructors.find { c -> !c.name.endsWith("empty", true) }
//
...
//                .addJavadoc("<ul>\n")
//        constructors.forEach { clazz.addJavadoc("<li>{@link ${constructorClassNameMap[it].toString().substringAfterLast('.')}}: ${it.name}#${it.id.hexString()}</li>\n") }
//        clazz.addJavadoc("</ul>\n\n")
//
//        if (constructor.forEmptyConstructor) {
...
//                                   .initializer("0x${id.hexString()}").build())
//
...
//                else -> ParameterizedTypeName.get(TYPE_TL_VECTOR, getType(type.parameters.first(), true).box())
//            }
//        }
//        is TLTypeRaw -> {
//            val name = type.name
